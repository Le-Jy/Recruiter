...
public class GradeHandler {
    ...
    public Student getStudent(String name) throws EntryNotFoundException {
        return students.stream().filter(s -> s.name().compareTo(name) == 0).findFirst().orElseThrow(() -> new EntryNotFoundException(Student.class, name));
    }
    ...
    public void updateStudent(String name, Function<Student, Student> function) throws EntryNotFoundException {
        students.add(function.apply(removeStudent(name)));
    }
    ...
    public double getStudentAverageInSubject(String name, Subject subject) throws EntryNotFoundException {
        return getStudent(name).grades().stream().filter(G -> G.activity().subject().equals(subject)).mapToDouble(Grade::grade).average().orElseThrow(() -> new EntryNotFoundException(Student.class, name));
    }
}
