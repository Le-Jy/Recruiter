...
namespace inlining
{
  ...
  void Inliner::operator()(const ast::CallExp& e)
  {
    parse::Tweast in;
    in << "let ";
    const ast::FunctionDec* func = e.def_get();
    const ast::VarChunk& formals = func->formals_get();
    const ast::exps_type& args = e.args_get();
    for (size_t i = 0; i < args.size(); i++)
      {
        in << *formals[i] << " := " << *args[i] << " ";
      }
    in << "var res : " << *func->result_get() << " := " << *func->body_get();
    in << " in res end";

    auto ast = parse::parse(in);
    if (ast.index() == 0)
