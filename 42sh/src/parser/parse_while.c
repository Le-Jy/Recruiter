 ...
 * @return A boolean.
 */
static bool follow_while(struct token *token)
{
    return (
...
bool first_while(struct token *token)
{
    return (token->type == TOKEN_WHILE);
}

static enum parser_status parse_while_case(struct ast **res,
                                           struct lexer *lexer)
{
    ...
    if (!follow_while(peek_token(lexer)))
        return PARSER_UNEXPECTED_TOKEN;

    return PARSER_OK;
}

enum parser_status parse_while(struct ast **res, struct lexer *lexer)
{
    ...
    if (current_token->type == TOKEN_WHILE)
    {
        ...
        return parse_while_case(res, lexer);
    }
    token_free(current_token);
