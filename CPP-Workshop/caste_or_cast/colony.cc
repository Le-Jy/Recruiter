        ...
        else
        {
            ...
            if (worker->food_level_ >= 4.0)
            {
                worker->stage_ = static_cast<DevelopmentStage>(
                    static_cast<int>(worker->stage_) + 1);
                worker->food_level_ = 0.f;
            }
        }
    }
...
{
    if (ant.stage_ == DevelopmentStage::ADULT
        && dynamic_cast<const Queen*>(&ant) && !colony->queen_)
    {
        colony->queen_ =
            std::make_unique<Queen>(dynamic_cast<const Queen&>(ant));
        return true;
    }
    else if (dynamic_cast<const Nurturer*>(&ant))
    {
        colony->workers_.push_back(
            std::make_shared<Nurturer>(dynamic_cast<const Nurturer&>(ant)));
        return true;
    }
    else if (dynamic_cast<const Provider*>(&ant))
    {
        colony->workers_.push_back(
